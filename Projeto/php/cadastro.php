        ...
        while(($dir = $d->read()) !== false){
            ...
            if($existe){
                ...
                if(!file_exists($url)){
                    ...
                    $xml_primeiro = $xml->createElement("primeiro");

                    $xml_nome = $xml->createElement("nome",$nome);
                    $xml_bv = $xml->createElement("Bem-vindo", "bem vindo");

                    $xml_primeiro->appendChild($xml_nome);
                    $xml_primeiro->appendChild($xml_bv);

                    $xml->appendChild($xml_primeiro);

                    $xml->save($url."/email/entrada/1.xml");
